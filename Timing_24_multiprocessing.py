...
def analyze_fibonacci_cycles(df, discovery_fractal_indices, tolerance_window=0.4, verbose=True):
    """
    ...
    validated_grids = []
    if verbose: print(f"Part 1: Analyzing fractal pairs (using tolerance_window={tolerance_window})...")
    disable_tqdm = not verbose or (len(fractal_indices) > 5000)
    
    iterator = range(len(fractal_indices))
    if not disable_tqdm: iterator = tqdm(iterator)
...
def discover_and_plot_good_cycles_without_drow(results_df, quantile_3=0.85, quantile_4=0.70, quantile_5=0.60, verbose=True):
...
def perform_advanced_validation(results_df, 
    ...
    if verbose: print(f"Filtered down to {len(filtered_results)} valid grids.")
    for ratio in fib_ratios:
        filtered_results[f'loc_{ratio}'] = filtered_results['Start'] + ratio * filtered_results['Length']
    
    if verbose: print("Part 2: Identifying forecast points...")
    all_forecasts_data = []
    ...
    if verbose: print("Part 2: Validating 'Enter Points'...")
    validated_points = []
...
def evaluate_individual_task(ind):
    ...
    # Execution Logic (Verbose=False)
    df_disc = find_fractals(worker_df_base.copy(), n=p_fractal_disc, verbose=False)
    disc_indices = df_disc.index[df_disc['Fractal'].notna()].tolist()
    
    df_val = find_fractals(worker_df_base.copy(), n=p_fractal_val, verbose=False)
    val_indices = df_val.index[df_val['Fractal'].notna()].tolist()
    
    results = analyze_fibonacci_cycles(df_disc, disc_indices, tolerance_window=p_grid_tolerance, verbose=False)
    
    kpi_value = -1000 
    if not results.empty:
        good_cycles = discover_and_plot_good_cycles_without_drow(results, quantile_3=p_q3, quantile_4=p_q4, quantile_5=p_q5, verbose=False)
        if good_cycles:
            enter_points_df, _ = perform_advanced_validation(results, val_indices, good_cycles, tolerance_window=p_grid_val_tolerance, verbose=False)
    ...
    # --- Mode 4: Optimum Search ---
        ...
        def create_individual():
            return {
                ...
                'QUANTILE_THRESHOLD_5': round(random.uniform(0.5, 0.9), 2),
