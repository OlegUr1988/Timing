...
# --- Stage 0, 1, 2, 3 Functions (Unchanged) ---
# ... (Keep load_cycles_from_file, save_cycles_to_file, load_real_data, find_fractals, analyze_fibonacci_cycles, discover_and_plot_good_cycles, perform_advanced_validation, check_fib_ratio_in_lengths functions here) ...
...
def fetch_recent_data(ticker, months=1.5):
    ...
    try:
        ...
        print(f"Successfully fetched {len(df)} recent data points.")
        return df
    except Exception as e:
...
def analyze_fibonacci_cycles(df, discovery_fractal_indices):
    ...
    validated_grids = []
    print("Analyzing fractal pairs (using discovery fractals) to find all potential grids...")
...
def discover_and_plot_good_cycles(results_df):
    ...
    if 'Total_Overlaps' not in results_df.columns: # Add if missing
...
def perform_advanced_validation(results_df, validation_fractal_indices, good_lengths):
    ...
    # Handle empty sorted_forecasts list
    ...
    for i in range(1, len(sorted_forecasts)):
        if sorted_forecasts[i]['location'] - current_cluster[0]['location'] <= 0.5:
            ...
            # --- START OF FIX 1 ---
            # Replace comprehension with a loop
            unique_starts = {}
            for forecast in current_cluster:
                start_id = forecast['start']
                # Keep only the forecast with the shortest length for each start_id
                if start_id not in unique_starts or forecast['length'] < unique_starts[start_id]['length']:
                    unique_starts[start_id] = forecast
            
            if len(unique_starts) >= 2: # Check if forecasts from >= 2 unique grids remain
                enter_points_clusters.append(list(unique_starts.values()))
            # --- END OF FIX 1 ---
            current_cluster = [sorted_forecasts[i]] # Start new cluster

    # Process the very last cluster
    if current_cluster: # Check if the last cluster is not empty
        # --- START OF FIX 2 ---
        unique_starts = {}
        for forecast in current_cluster:
            start_id = forecast['start']
...
# --- NEW FUNCTION FOR REAL-TIME PLOTTING (MODIFIED) ---
def plot_real_time_chart(df_recent, enter_points_recent_df, all_forecasts_recent_df, currency_pair, min_forecast_count=3, apply_filter_unique=False, apply_filter_no_fib=False):
    ...
    Applies filters based on flags. Highlights future points and adds info about the next 3.
    """
    ...
    future_points_hours = []
    now_time = pd.Timestamp.now().tz_localize(None) # Make now_time offset-naive like df_recent['Timestamp']

    # Add markers/lines for enter points
    for _, row in filtered_enter_points.iterrows():
        location = row['Enter_Point_Location']
        # Check if location index is within the bounds of df_recent
        if not (0 <= location < len(df_recent)):
            print(f"Warning: Enter_Point_Location {location} is outside the range of recent data index [0, {len(df_recent)-1}]. Skipping point.")
            continue # Skip this point if its index is invalid for df_recent

        floor_index = int(location)
        ceil_index = floor_index + 1
        # Need to handle edge case where location is exactly the last index
        if ceil_index >= len(df_recent):
            if floor_index == len(df_recent) - 1: # If location is very close to the last index
                precise_timestamp = df_recent.iloc[floor_index]['Timestamp']
            else: # Should not happen if previous check works, but as safeguard
                 continue
        else:
            t1, t2 = df_recent.iloc[floor_index]['Timestamp'], df_recent.iloc[ceil_index]['Timestamp']
            fraction = location - floor_index
            precise_timestamp = t1 + ((t2 - t1) * fraction)
        
        # Ensure price_at_location index is valid
        price_index = int(round(location))
        if not (0 <= price_index < len(df_recent)):
             price_index = floor_index # Fallback to floor index
        price_at_location = df_recent.iloc[price_index]['Close']

        # Check if point is in the future relative to the LATEST data point
        latest_data_time = df_recent['Timestamp'].iloc[-1]
        if precise_timestamp > latest_data_time:
            ...
            if hours_diff > 0:
                future_points_hours.append(hours_diff)
        else:
...
if __name__ == '__main__':
    # --- Configuration ---
    # data_file = "EURUSD_Hourly_Bid_2025.01.01_2025.09.16.csv" # Used only in FULL mode if needed
    ...
    # --- Mode 3: Real-Time Forecast ---
        ...
        if enter_points_recent_df.empty: print("Could not generate Enter Points from recent data."); exit()

        # Call the new real-time plotting function
        plot_real_time_chart(
            ...
            apply_filter_unique=APPLY_FILTER_UNIQUE_LENGTHS, # Pass flags
            apply_filter_no_fib=APPLY_FILTER_NO_FIB_RATIO    # Pass flags
            )
...
# --- Stage 0, 1, 2, 3 Functions (Unchanged) ---
# ... (Keep load_cycles_from_file, save_cycles_to_file, load_real_data, find_fractals, analyze_fibonacci_cycles, discover_and_plot_good_cycles, perform_advanced_validation, check_fib_ratio_in_lengths functions here) ...
# (Functions are omitted for brevity, but they should be included in your script)
def load_cycles_from_file(filename):
    try:
        with open(filename, 'r') as f: return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError): return {}
...
def plot_real_time_chart(df_recent, enter_points_recent_df, all_forecasts_recent_df, currency_pair, min_forecast_count=3, apply_filter_unique=False, apply_filter_no_fib=False):
    ...
    if enter_points_recent_df.empty:
        print("Received empty Enter Points DataFrame. Cannot plot real-time chart.")
        return
    ...
    if apply_filter_unique:
        print("Applying Unique Length Filter for Real-Time Chart...")
        if not df_to_plot.empty: # Only filter if there's data to filter
            indices_unique = [idx for idx, row in df_to_plot.iterrows() if all_forecasts_recent_df[all_forecasts_recent_df['Forecast_ID'].isin(row['Contributing_Forecast_IDs'])]['Base_Cycle_Length'].nunique() == len(row['Contributing_Forecast_IDs'])]
            df_to_plot = df_to_plot.loc[indices_unique]
            applied_filters_names.append("Unique Lengths")
            print(f"After Filter 1: {len(df_to_plot)} Enter Points remain.")
        else:
            print("Skipping Unique Length Filter as DataFrame is already empty.")
    ...
    if apply_filter_no_fib:
        ...
        if not df_to_plot.empty: # Only filter if there's data to filter
            ...
            print(f"After Filter 2: {len(df_to_plot)} Enter Points remain.")
             ...
             print("Skipping No Fibo Ratio Filter as DataFrame is already empty.")
    # --- ---
    
    # Check if dataframe is empty *after* filtering
    if df_to_plot.empty:
        print(f"No Enter Points remaining after applying filters. Cannot plot.")
        return
    ...
    for _, row in filtered_enter_points.iterrows():
        location = row['Enter_Point_Location']
        if not (0 <= location < len(df_recent)):
            # This is a forecast point *beyond* the fetched data, we must extrapolate time
            if location >= len(df_recent):
                last_time = df_recent['Timestamp'].iloc[-1]
                # Assuming 1 hour intervals
                time_diff = datetime.timedelta(hours=(location - (len(df_recent) - 1)))
                precise_timestamp = last_time + time_diff
                price_at_location = df_recent['Close'].iloc[-1] # Plot at last known price
            else:
                continue # Skip invalid negative index
        else:
            # Point is within the fetched data range
            floor_index = int(location)
            ceil_index = floor_index + 1
            if ceil_index >= len(df_recent):
                precise_timestamp = df_recent.iloc[floor_index]['Timestamp']
                price_at_location = df_recent.iloc[floor_index]['Close']
            else:
                t1, t2 = df_recent.iloc[floor_index]['Timestamp'], df_recent.iloc[ceil_index]['Timestamp']
                fraction = location - floor_index
                precise_timestamp = t1 + ((t2 - t1) * fraction)
                price_at_location = df_recent.iloc[int(round(location))]['Close']

        # Check if point is in the future
        if precise_timestamp > latest_data_time:
            fig.add_vline(x=precise_timestamp, line_width=1.5, line_dash="dot", line_color="blue")
            time_diff = precise_timestamp - now_time
            hours_diff = time_diff.total_seconds() / 3600
...
if __name__ == '__main__':
    ...
    # --- Mode 3: Real-Time Forecast (MODIFIED) ---
    elif EXECUTION_MODE == 'REAL_TIME':
        print("\n--- Running in REAL-TIME Forecast Mode ---")
        cycles_db = load_cycles_from_file(CYCLES_DATABASE_FILE)
        if currency_pair not in cycles_db: print(f"Error: No saved settings for {currency_pair.upper()}. Run 'FULL' mode first."); exit()
        ...
        # --- THIS IS THE FIX ---
        # Only proceed to plotting if Enter Points were actually found
        if enter_points_recent_df.empty:
            print("Could not generate Enter Points from recent data. No chart to display.")
        else:
            # Now it is safe to call the plot function
            plot_real_time_chart(
                df_recent, 
                enter_points_recent_df, 
                all_forecasts_recent_df, # Pass all_forecasts for filtering
                currency_pair,
                min_forecast_count=MIN_FORECAST_COUNT_FOR_CHART,
...
def fetch_recent_data(ticker, months=1.5):
    """Fetches recent 1-hour data using yfinance."""
    print(f"\nFetching recent ~{months} months of 1-hour data for {ticker}...")
    end_date = datetime.datetime.now()
    approx_days = int(months * 30.44) # Use average days in a month
    start_date = end_date - pd.DateOffset(days=approx_days)
    print(f"Calculated start date: {start_date.strftime('%Y-%m-%d')}")
    try:
        yf_ticker = f"{ticker[:3]}{ticker[3:]}=X"
...
# --- Stage 4: Visualization Functions (Unchanged) ---
def check_fib_ratio_in_lengths(lengths, tolerance=0.04):
    fib_check_ratios = {0.382, 0.618, 1.618, 2.618, 4.236}
    if len(lengths) < 2: return False
    for l1, l2 in combinations(lengths, 2):
...
def plot_real_time_chart(df_recent, enter_points_recent_df, all_forecasts_recent_df, currency_pair, min_forecast_count=3, apply_filter_unique=False, apply_filter_no_fib=False):
    """
    Plots recent price data and overlays past/future Enter Points.
    (NumPy-corrected version 2.0 to prevent ambiguity errors)
    """
    print(f"\nReal-Time Mode: Generating price chart with Enter Points (Forecast Count >= {min_forecast_count})...")
    ...
    if apply_filter_unique:
        ...
        if not df_to_plot.empty:
            ...
            applied_filters_names.append("Unique Lengths")
            print(f"After Filter 1: {len(df_to_plot)} Enter Points remain.")
        else: print("Skipping Unique Length Filter as DataFrame is already empty.")
    ...
    if apply_filter_no_fib:
        ...
        if not df_to_plot.empty:
            ...
            if "No Fibo Ratio Lengths" not in applied_filters_names:
                applied_filters_names.append("No Fibo Ratio Lengths")
            print(f"After Filter 2: {len(df_to_plot)} Enter Points remain.")
        else: print("Skipping No Fibo Ratio Filter as DataFrame is already empty.")
    ...
    latest_data_time_np = timestamps_np[-1] # Get last timestamp
    ...
    if filtered_enter_points.empty:
        ...
        for _, row in filtered_enter_points.iterrows():
            ...
            # --- This is the scalar price for the marker's Y-position ---
            price_index = int(round(location))
            # Ensure price_index is within bounds, fallback to last known price
            if not (0 <= price_index < num_rows):
                price_at_location = close_prices_np[-1]
            else:
                price_at_location = close_prices_np[price_index]

            # --- This logic block calculates the scalar precise_timestamp ---
            if not (0 <= location < num_rows):
                # Point is outside the fetched data (likely in the future)
                ...
                if ceil_index >= num_rows: # At the very last candle
                    precise_timestamp = pd.Timestamp(timestamps_np[floor_index])
                else:
                    # Interpolate time
                    t1 = pd.Timestamp(timestamps_np[floor_index])
                    t2 = pd.Timestamp(timestamps_np[ceil_index])
                    fraction = location - floor_index
                    precise_timestamp = t1 + ((t2 - t1) * fraction)
            # --- End of precise_timestamp calculation ---

            
            # This comparison is now between two scalar timestamp objects
            if precise_timestamp > pd.Timestamp(latest_data_time_np):
                fig.add_vline(x=precise_timestamp, line_width=1.5, line_dash="dot", line_color="blue")
                time_diff = precise_timestamp - now_time
                hours_diff = time_diff.total_seconds() / 3600
