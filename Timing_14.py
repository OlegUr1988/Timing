...
def save_cycles_to_file(filename, data):
    ...
    print(f"\nSaved/Updated cycle data for the current pair in '{filename}'") # Minor text change
...
def plot_filtered_success_rate_comparison(original_df, filtered_df, currency_pair, filter_name="Filtered"):
    ...
    if original_df.empty:
        print("Original DataFrame is empty. Cannot generate comparison plot."); return
...
# --- All functions (load_cycles_from_file, save_cycles_to_file, load_real_data, find_fractals, analyze_fibonacci_cycles, discover_and_plot_good_cycles, perform_advanced_validation, check_fib_ratio_in_lengths, plot_filtered_success_rate_comparison, plot_price_chart_with_enter_points) remain unchanged ---
# ... (Keep all the previous functions here) ...
...
if __name__ == '__main__':
    ...
    # --- Define the fractal levels ---
    FRACTAL_LEVEL_DISCOVERY = 3
    FRACTAL_LEVEL_VALIDATION = 3

    # --- Define the minimum forecast count for the final chart ---
    ...
    if df is not None:
        ...
        if not results.empty:
            ...
            # Store as a dictionary containing all relevant info
            cycles_db[currency_pair] = {
                "good_cycles": good_cycle_lengths,
                "discovery_fractal_level": FRACTAL_LEVEL_DISCOVERY,
                "min_forecast_count_chart": MIN_FORECAST_COUNT_FOR_CHART # Added this line
            }
            ...
            if not good_cycle_lengths:
                ...
                if not enter_points_df.empty:
                    ...
                    # Apply Filters and Plot (unchanged logic, just uses the variable now)
                    ...
                    plot_filtered_success_rate_comparison(enter_points_df, filtered_ep_no_fib_ratio, currency_pair, filter_name="No Fibo Ratio Lengths")

                    # Use the variable here
                    plot_price_chart_with_enter_points(df, enter_points_df, currency_pair, min_forecast_count=MIN_FORECAST_COUNT_FOR_CHART)
                else:
