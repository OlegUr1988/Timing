...
if __name__ == '__main__':
    ...
    # --- Define fractal levels ---
    FRACTAL_LEVEL_DISCOVERY = 3
    FRACTAL_LEVEL_VALIDATION = 3
    
    # --- NEW: Filter Control Flags ---
    APPLY_FILTER_UNIQUE_LENGTHS = True # Set to True or False
    APPLY_FILTER_NO_FIB_RATIO = True # Set to True or False
    # --- ---
    
    # Minimum forecast count for the final price chart
    ...
    if df is not None:
        ...
        if not results.empty:
            ...
            cycles_db[currency_pair] = { "good_cycles": good_cycle_lengths, "discovery_fractal_level": FRACTAL_LEVEL_DISCOVERY }
            ...
            if not good_cycle_lengths:
                ...
                if not all_forecasts_df.empty:
                    ...
                    print(f"Overall individual forecast hit-rate (using n={FRACTAL_LEVEL_VALIDATION}): {hit_rate:.2f}%")
                    print("-" * 40)
                ...
                if not enter_points_df.empty:
                    ...
                    df_for_price_chart = enter_points_df.copy() # Start with original
                    applied_filters_names = []

                    # Filter 1: Unique Lengths
                    if APPLY_FILTER_UNIQUE_LENGTHS:
                        ...
                        for index, row in enter_points_df.iterrows():
                            ...
                            if contributing_forecasts['Base_Cycle_Length'].nunique() == len(contributing_forecasts): indices_to_keep_unique.append(index)
                    ...
                    # Filter 2: No Fibo Ratio Lengths
                    if APPLY_FILTER_NO_FIB_RATIO:
                        ...
                        # IMPORTANT: Apply this filter to the *current* df_for_price_chart
                        for index, row in df_for_price_chart.iterrows(): 
                            forecast_ids = row['Contributing_Forecast_IDs']
                            # Need all_forecasts_df to get lengths
                            contributing_forecasts = all_forecasts_df[all_forecasts_df['Forecast_ID'].isin(forecast_ids)]
                            lengths = contributing_forecasts['Base_Cycle_Length'].tolist()
                            if not check_fib_ratio_in_lengths(lengths): indices_to_keep_no_fib.append(index)
                        
                        # Create a separate df just for the comparison plot
                        filtered_ep_no_fib_ratio_comp = enter_points_df.loc[indices_to_keep_no_fib].copy() # Compare vs original
                        
                        # Apply filter to the dataframe for the price chart
                        df_for_price_chart = df_for_price_chart.loc[indices_to_keep_no_fib].copy() 
                        applied_filters_names.append("No Fibo Ratio Lengths")
                        print(f"Filter 2 Result: {len(df_for_price_chart)} Enter Points remain after this filter.")

                        # Plot comparison only if filter is applied (Compare this filter result vs original)
                    ...
                    # --- Call Plots ---
                    # The success rate plot always shows the overall rate from the original enter_points_df
                    # plot_enter_point_success_rate(enter_points_df, currency_pair) # Re-enable if you want the stacked chart too
                    
                    # The price chart uses the sequentially filtered data
                    plot_price_chart_with_enter_points(
                        df, 
                        df_for_price_chart, # Pass the final filtered df
                        currency_pair, 
                        min_forecast_count=MIN_FORECAST_COUNT_FOR_CHART,
                        applied_filters_names=applied_filters_names # Pass filter names
                    )
                else:
...
# --- Main Execution (MODIFIED FOR ALWAYS PLOTTING COMPARISONS) ---
if __name__ == '__main__':
    # --- Configuration ---
    data_file = "EURUSD_Hourly_Bid_2025.01.01_2025.09.16.csv"
    data_file = "USDCHF_Hourly_Bid_2024.01.01_2025.10.10.csv"
    ...
    # These now control ONLY the sequential filtering for the final price chart
    # and which filters are included in the 'Combined' comparison.
    APPLY_FILTER_UNIQUE_LENGTHS = True # Set to True or False
    APPLY_FILTER_NO_FIB_RATIO = True # Set to True or False
    # --- ---
        ...
        if not results.empty:
            ...
            if not good_cycle_lengths:
                ...
                if not all_forecasts_df.empty:
                    print("\n--- DETAILED FORECAST ANALYSIS ---")
                    hit_rate = all_forecasts_df['Has_Fractal_Overlap'].mean() * 100
                    print(f"Overall individual forecast hit-rate (validated with n={FRACTAL_LEVEL_VALIDATION}): {hit_rate:.2f}%")
                    print("-" * 40)
                ...
                if not enter_points_df.empty:
                    ...
                    # --- ALWAYS Calculate Individual Filter Results ---
                    # Filter 1: Unique Lengths (for comparison plot 1)
                    print("\nCalculating Filter 1 (Unique Lengths)...")
                    indices_to_keep_unique = []
                    for index, row in enter_points_df.iterrows():
                        forecast_ids = row['Contributing_Forecast_IDs']
                        contributing_forecasts = all_forecasts_df[all_forecasts_df['Forecast_ID'].isin(forecast_ids)]
                    ...
                    filtered_ep_unique_length = enter_points_df.loc[indices_to_keep_unique].copy()
                    print(f"Result for Comparison Plot 1: {len(filtered_ep_unique_length)} Enter Points remain.")

                    # Filter 2: No Fibo Ratio Lengths (for comparison plot 2)
                    print("\nCalculating Filter 2 (No Fibo Ratio Lengths)...")
                    indices_to_keep_no_fib = []
                    for index, row in enter_points_df.iterrows():
                        forecast_ids = row['Contributing_Forecast_IDs']
                        contributing_forecasts = all_forecasts_df[all_forecasts_df['Forecast_ID'].isin(forecast_ids)]
                        lengths = contributing_forecasts['Base_Cycle_Length'].tolist()
                        if not check_fib_ratio_in_lengths(lengths):
                            indices_to_keep_no_fib.append(index)
                    filtered_ep_no_fib_ratio_comp = enter_points_df.loc[indices_to_keep_no_fib].copy()
                    print(f"Result for Comparison Plot 2: {len(filtered_ep_no_fib_ratio_comp)} Enter Points remain.")

                    # --- Sequentially Apply SELECTED Filters for Final Chart & Comparison Plot 3 ---
                    df_for_price_chart = enter_points_df.copy() # Start with original
                    applied_filters_names = []
                    print("\nApplying selected filters sequentially for final price chart...")

                    if APPLY_FILTER_UNIQUE_LENGTHS:
                        df_for_price_chart = df_for_price_chart.loc[indices_to_keep_unique].copy()
                        applied_filters_names.append("Unique Lengths")
                        print(f"After Filter 1: {len(df_for_price_chart)} Enter Points remain.")

                    if APPLY_FILTER_NO_FIB_RATIO:
                         # Apply filter 2 to the *already potentially filtered* df_for_price_chart
                        indices_to_keep_no_fib_seq = []
                        for index, row in df_for_price_chart.iterrows(): # Iterate the *current* df
                             forecast_ids = row['Contributing_Forecast_IDs']
                             contributing_forecasts = all_forecasts_df[all_forecasts_df['Forecast_ID'].isin(forecast_ids)]
                             lengths = contributing_forecasts['Base_Cycle_Length'].tolist()
                             if not check_fib_ratio_in_lengths(lengths):
                                 indices_to_keep_no_fib_seq.append(index)
                        df_for_price_chart = df_for_price_chart.loc[indices_to_keep_no_fib_seq].copy()
                        applied_filters_names.append("No Fibo Ratio Lengths")
                        print(f"After Filter 2: {len(df_for_price_chart)} Enter Points remain.")

                    # --- ALWAYS CALL THE COMPARISON PLOTS ---
                    ...
                    plot_filtered_success_rate_comparison(
                        enter_points_df, filtered_ep_no_fib_ratio_comp, currency_pair,
                        filter_name="No Fibo Ratio Lengths"
                    )
                    # --- NEW THIRD COMPARISON PLOT ---
                    combined_filter_name = "Combined Filters" if applied_filters_names else "No Filters Applied"
                    plot_filtered_success_rate_comparison(
                        enter_points_df, df_for_price_chart, currency_pair, # Compare Original vs Final
                        filter_name=combined_filter_name
                    )
                    # --- ---

                    # --- Call Price Chart Plot ---
                    plot_price_chart_with_enter_points(
                        df,
                        df_for_price_chart, # Pass the final sequentially filtered df
                        currency_pair,
                        min_forecast_count=MIN_FORECAST_COUNT_FOR_CHART,
                        applied_filters_names=applied_filters_names # Pass filter names
                    )
...
# --- Main Execution (CORRECTED Filtering Logic) ---
if __name__ == '__main__':
    # --- Configuration ---
    data_file = "EURUSD_Hourly_Bid_2025.01.01_2025.09.16.csv"
    CYCLES_DATABASE_FILE = "good_cycles_database.json"
    ...
    # --- Filter Control Flags ---
    APPLY_FILTER_UNIQUE_LENGTHS = False # Set to True or False
    APPLY_FILTER_NO_FIB_RATIO = False # Set to True or False
    # --- ---
    ...
    if df is not None:
        ...
        if not results.empty:
            ...
            if not good_cycle_lengths:
                ...
                print(f"\nUsing newly discovered cycle lengths (found with n={FRACTAL_LEVEL_DISCOVERY}) for Part 2 analysis: {good_cycle_lengths}")
                ...
                if not enter_points_df.empty:
                    ...
                    # --- Prepare DataFrames for Filters ---
                    df_for_price_chart = enter_points_df.copy() # Start with original for sequential filtering
                    applied_filters_names = []
                    filtered_ep_unique_length = pd.DataFrame() # Initialize for comparison plot
                    filtered_ep_no_fib_ratio_comp = pd.DataFrame() # Initialize for comparison plot

                    # --- Filter 1 Calculation (Always calculate for potential comparison) ---
                    print("\nCalculating Filter 1 (Unique Lengths) results...")
                    indices_to_keep_unique = []
                    for index, row in enter_points_df.iterrows():
                        forecast_ids = row['Contributing_Forecast_IDs']
                        contributing_forecasts = all_forecasts_df[all_forecasts_df['Forecast_ID'].isin(forecast_ids)]
                        if contributing_forecasts['Base_Cycle_Length'].nunique() == len(contributing_forecasts):
                            indices_to_keep_unique.append(index)
                    filtered_ep_unique_length = enter_points_df.loc[indices_to_keep_unique].copy()
                    print(f"Filter 1 Result (for comparison): {len(filtered_ep_unique_length)} Enter Points remain.")

                    # Apply Filter 1 sequentially if flag is True
                    if APPLY_FILTER_UNIQUE_LENGTHS:
                        df_for_price_chart = df_for_price_chart.loc[indices_to_keep_unique].copy()
                        applied_filters_names.append("Unique Lengths")
                        print(f"Applied Filter 1 sequentially: {len(df_for_price_chart)} Enter Points remain.")

                    # --- Filter 2 Calculation (Always calculate for potential comparison) ---
                    print("\nCalculating Filter 2 (No Fibo Ratio Lengths) results...")
                    indices_to_keep_no_fib_comp = []
                    for index, row in enter_points_df.iterrows(): # Based on original
                        forecast_ids = row['Contributing_Forecast_IDs']
                        contributing_forecasts = all_forecasts_df[all_forecasts_df['Forecast_ID'].isin(forecast_ids)]
                        lengths = contributing_forecasts['Base_Cycle_Length'].tolist()
                        if not check_fib_ratio_in_lengths(lengths):
                            indices_to_keep_no_fib_comp.append(index)
                    filtered_ep_no_fib_ratio_comp = enter_points_df.loc[indices_to_keep_no_fib_comp].copy()
                    print(f"Filter 2 Result (for comparison): {len(filtered_ep_no_fib_ratio_comp)} Enter Points remain.")

                    # Apply Filter 2 sequentially if flag is True
                    if APPLY_FILTER_NO_FIB_RATIO:
                        # --- THIS IS THE FIX: Calculate indices based on the CURRENT df_for_price_chart ---
                        indices_to_keep_no_fib_seq = []
                        for index, row in df_for_price_chart.iterrows(): # Iterate the potentially already filtered df
                            forecast_ids = row['Contributing_Forecast_IDs']
                            contributing_forecasts = all_forecasts_df[all_forecasts_df['Forecast_ID'].isin(forecast_ids)]
                            lengths = contributing_forecasts['Base_Cycle_Length'].tolist()
                            if not check_fib_ratio_in_lengths(lengths):
                                indices_to_keep_no_fib_seq.append(index)
                        # --- ---
                        df_for_price_chart = df_for_price_chart.loc[indices_to_keep_no_fib_seq].copy() # Apply using the correct indices
                        applied_filters_names.append("No Fibo Ratio Lengths")
                        print(f"Applied Filter 2 sequentially: {len(df_for_price_chart)} Enter Points remain.")


                    # --- ALWAYS CALL THE COMPARISON PLOTS (using the separately calculated DFs) ---
                    plot_filtered_success_rate_comparison(
                        enter_points_df, filtered_ep_unique_length, currency_pair,
                        filter_name="Unique Cycle Lengths"
                    )
                    ...
                    # --- Plot comparison for COMBINED selected filters ---
                    combined_filter_name = " & ".join(applied_filters_names) if applied_filters_names else "No Filters Applied"
                    # Only plot if at least one filter was applied
                    if applied_filters_names:
                         plot_filtered_success_rate_comparison(
                             enter_points_df, df_for_price_chart, currency_pair, # Compare Original vs Final Sequentially Filtered
                             filter_name=combined_filter_name
                         )
