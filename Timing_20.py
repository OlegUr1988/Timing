...
def plot_real_time_chart(df_recent, enter_points_recent_df, all_forecasts_recent_df, currency_pair, min_forecast_count=3, apply_filter_unique=False, apply_filter_no_fib=False):
    ...
    last_candle_index = num_rows - 1
    last_candle_time = pd.Timestamp(timestamps_np[last_candle_index])
    ...
    if filtered_enter_points.empty:
        ...
        for _, row in filtered_enter_points.iterrows():
            ...
            if location > last_candle_index:
                # --- THIS IS A FUTURE POINT ---
                # Extrapolate time assuming 1 hour intervals
                hours_past_last_candle = location - last_candle_index
                time_diff = datetime.timedelta(hours=hours_past_last_candle)
                precise_timestamp = last_candle_time + time_diff
                
                fig.add_vline(x=precise_timestamp, line_width=1.5, line_dash="dot", line_color="blue")
                
                # Calculate hours from NOW
                time_diff_from_now = precise_timestamp - now_time
                hours_diff = time_diff_from_now.total_seconds() / 3600
                if hours_diff > 0: future_points_hours.append(hours_diff)
                
            elif 0 <= location <= last_candle_index:
                # --- THIS IS A PAST POINT ---
                ...
                if ceil_index >= num_rows:
                    ...
                    price_at_location = close_prices_np[floor_index]
                else:
            ...
            # else: location is < 0, so we ignore it
